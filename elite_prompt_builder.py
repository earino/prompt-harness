...
class ElitePromptBuilder:
    ...
    def generate_prompt(self, spec: dict) -> str:
        ...
        try:
            ...
            # Extract just the prompt if it's wrapped in code blocks
            match = re.search(r'```\n([\s\S]*?)\n```', draft)
            if match:
                draft = match.group(1)
...
Please refine the prompt to address the critique. Incorporate the specific improvements suggested while maintaining the prompt's core purpose. Output ONLY the refined prompt text (no explanations, no preamble), ready to use in a code block."""
